"""
graph.py — Orquestación del RAG clínico (guías VIH) con LangGraph + LangSmith.

Reutiliza TAL CUAL las funciones del pipeline existente (main.py / evidencias.py)
y las encadena como nodos de un grafo, para poder auditar cada paso en LangSmith.
...
Uso:
    python graph.py            # modo interactivo (como main.py)
"""
import os
import sys
...
import main  # crea los clientes OpenAI/Qdrant al importarse (no ejecuta main())
from main import retrieve, build_context, SYS_PROMPT, build_user_prompt
from evidencias import format_answer

# --- Trazado fino de las llamadas a OpenAI (tokens, latencia) SIN modificar main.py ---
# Las funciones de main leen `main.client` en tiempo de ejecución, así que reasignar
# aquí su versión "envuelta" por LangSmith basta para que retrieve()/generate_answer()
# queden trazadas dentro del run del grafo.
if TRACING:
    try:
        from langsmith.wrappers import wrap_openai
        main.client = wrap_openai(main.client)
    except Exception:
        pass
