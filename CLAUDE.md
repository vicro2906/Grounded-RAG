...
Punto de entrada: `main.py` (grafo compilado `app`). Pipeline de 5 nodos:

```
question ─▶ rephrase ─▶ retrieve (híbrido) ─▶ rerank ─▶ generate ⇄ validate ─▶ evidence ─▶ output
                                                              (no apto+agotado ─▶ fallback)
```

- **rephrase** (`rag.rephrase`, gpt-4o-mini): reescribe la consulta SIN añadir info y
  normaliza términos incluyendo AMBAS formas «nombre completo (SIGLA)» usando
  `abreviaturas.py`. La generación usa la pregunta ORIGINAL, no la reescrita.
- **retrieve** (`rag.retrieve_hibrido`): búsqueda híbrida densa (OpenAI
...
- **validate** (`rag.validar`, gpt-4o-mini): juez de relevancia + grounding semántico.
  Bucle con `generate` (inyecta feedback al reintentar), `MAX_ITER=2`. apto → evidence;
  no apto y agotado → `fallback` (mensaje seguro, no se muestra la respuesta).
- **evidence** (`evidencias.format_answer`): formatea respuesta + panel de fuentes con
  citas literales (fuzzy match) + preguntas de seguimiento. Texto limpio (sin ANSI).

`rag.buscar()` = rephrase → híbrido → rerank (lo usa la evaluación).

## Ficheros clave

- `main.py` — grafo LangGraph + LangSmith + generación estructurada (punto de entrada).
- `rag.py` — pipeline: clientes, embeddings, retrieve/retrieve_hibrido, rerank, rephrase,
  buscar, generate_answer (versión cruda), SYS_PROMPT, build_user_prompt, constantes de modelo.
- `evidencias.py` — formateo de respuesta y fuentes.
- `evaluacion_rag.py` — evaluación RAGAS (golden set de 47 preguntas; `RETRIEVER=buscar`).
- `abreviaturas.py` — diccionario SIGLA→nombre de las guías.
...
- Generación: **gpt-4o** (`MODELO_GENERACION`). Rephrase: **gpt-4o-mini** (`MODELO_REPHRASE`).
...
- Evaluación RAGAS: `.venv\Scripts\python.exe evaluacion_rag.py` (usa el pipeline completo).
...
- **F3 — Refine: CASI HECHA.** Hecho: rephrasing + normalización de abreviaturas, y el
  **bloque Validate** (nodo `validate` tras `generate`, juez gpt-4o-mini de relevancia +
  grounding semántico; bucle con reintento inyectando feedback, `MAX_ITER=2`; si se agota
  sin respuesta apta → nodo `fallback` con mensaje seguro, NO se muestra la respuesta). El
  validador NO re-chequea citas literales (eso ya lo hace `evidence`). Pendiente menor:
  guardrail de entrada (clasificador de intención) y clasificación de pregunta.
